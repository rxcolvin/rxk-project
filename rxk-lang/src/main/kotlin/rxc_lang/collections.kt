...
class M<K, V> private constructor(args: Map<K,V>) : Map<K,V> by args {
    ...
    class B<K, V> : Collecter<Pair<K,V>, M<K,V>> {
...
interface Collecter<R, X> {
    fun add(t:R)
    fun build(): X
}

public inline fun <T, R, X, C : Collecter <in R, X>> Iterable<T>.map(builder: C, transform: (T) -> R): X {
    for (item in this)
